...
class KeyboardController extends Controller
{
    ...
    public function getkeyboard(Request $request){
        $columns=[
            0=>"id",
            1=>"brand",
            2=>"configuration",
        ...
        $recordsTotal=Asset::where('accessories_type','3')->count();
        ...
        if(!empty($record)){
            foreach($record as $value){
                $assign=Assign::where('item_id',$value->id)->where('type','5')->first()->assign;
                ...
                <li>
                <a data-bs-toggle='modal' class='asso-assignhistory' data-id='".$value->id."' data-type='5' data-heading='keyboard Assign' data-bs-target='#assign_device_histroy_popup'>
    ...
    //keyboard save
    public function keyboardSave(Request $request){
        $keyboarddata=Asset::where('accessories_type','=','3')->orderBy('id','desc')->first();
        
        if(empty($keyboarddata->systemid)){
            $systemid="01";
        }else{
            $systemid=$keyboarddata->systemid+1;
            if($systemid <10){
                $systemid="0".$systemid;
            }
        }
        ...
        if($file=$request->has('device_pic')){
            ...
            $file->move(public_path('/keyboard'),$device_pic);
        }

        if($file1=$request->has('invoice_file')){
            $file1=$request->file('invoice_file');
            $invoice_file=time().'-'.$file1->getClientOriginalName();
            $file1->move(public_path('/keyboard'),$invoice_file);
        }

        $keyboard=new Asset;
        $keyboard->storage=$request->input('storage');
        $keyboard->systemid=$systemid;
        $keyboard->brand=$request->input('brand');
        $keyboard->configuration=$request->input('configuration');
        $keyboard->serial_no=$request->input('serial_no');
        $keyboard->status=$request->input('status');
        $keyboard->warranty_end=$request->input('warranty_end');

        $keyboard->invoice_no=$request->input('invoice_no');
        $keyboard->invoice_date=$request->input('invoice_date');
        $keyboard->invoice_company_name=$request->input('invoice_company_name');

        $keyboard->accessoriesName="keyboard";
        $keyboard->accessoriesCategory="Accessories";

        $keyboard->device_pic=$device_pic ?? '';
        $keyboard->invoice_file=$invoice_file ?? '';
        $keyboard->accessories_type='3';
        //$keyboard->system_type='1';
        $keyboard=$keyboard->save();
        if($keyboard){
            $keyboard_data=Asset::orderBy('id','desc')->first()->id;
            ...
            $assign=$assign->save();
            if($assign){
                return back()->with('msg',"keyboard Add Successfully");
            }else{
                return back()->with('err_msg',"Some Error");
            }
            
        }else{
            return back()->with('err_msg',"Some Error in keyboard Add");
        }
    }


    //keyboard assign
    public function keyboardAssign(Request $request){
    ...
    public function productDetail($id){
        ...
        $data->join('employee','employee.id','=','assign_table.assign');
        $data->where('assign_table.type','5');
        $data->where('ram.accessories_type','3');
