...
class GraphiccardController extends Controller
{
    ...
    public function graphiccard(){
        ...
        $total=Asset::where('accessories_type','5')->count();
        $used=Assign::distinct('item_id')->where('type',7)->where('assign','!=','2')->count();
        
        return view('graphiccard',compact('total','used'));
    }

    public function getgraphiccard(Request $request){
        $columns=[
            0=>"id",
            1=>"brand",
            2=>"configuration",
        ...
        $recordsTotal=Asset::where('accessories_type','5')->count();
        ...
        if(!empty($record)){
            foreach($record as $value){
                $assign=Assign::where('item_id',$value->id)->where('type','7')->first()->assign;
                ...
                <a data-bs-toggle='modal' class='assignclick' data-id='".$value->id."' data-type='7' data-heading='graphiccard Assign' data-bs-target='#assign_device_popup'>
                <img src='".basepath('images/table-image/admin.svg')."'>
                </a>
                </li>
                <li>
                <a data-bs-toggle='modal' class='asso-assignhistory' data-id='".$value->id."' data-type='7' data-heading='graphiccard Assign' data-bs-target='#assign_device_histroy_popup'>
                ...
                $result['serial_no']="<a href='/product-detail/$value->id/5/7'>".$value->serial_no."</a>";
    ...
    //graphiccard save
    public function graphiccardSave(Request $request){
        $graphiccarddata=Asset::where('accessories_type','=','5')->orderBy('id','desc')->first();
        
        if(empty($graphiccarddata->systemid)){
            $systemid="01";
        }else{
            $systemid=$graphiccarddata->systemid+1;
            if($systemid <10){
                $systemid="0".$systemid;
            }
        }
        ...
        if($file=$request->has('device_pic')){
            ...
            $file->move(public_path('/graphiccard'),$device_pic);
        }

        if($file1=$request->has('invoice_file')){
            $file1=$request->file('invoice_file');
            $invoice_file=time().'-'.$file1->getClientOriginalName();
            $file1->move(public_path('/graphiccard'),$invoice_file);
        }

        $graphiccard=new Asset;
        $graphiccard->storage=$request->input('storage');
        $graphiccard->systemid=$systemid;
        $graphiccard->brand=$request->input('brand');
        $graphiccard->configuration=$request->input('configuration');
        $graphiccard->serial_no=$request->input('serial_no');
        $graphiccard->status=$request->input('status');
        $graphiccard->warranty_end=$request->input('warranty_end');

        $graphiccard->invoice_no=$request->input('invoice_no');
        $graphiccard->invoice_date=$request->input('invoice_date');
        $graphiccard->invoice_company_name=$request->input('invoice_company_name');

        $graphiccard->accessoriesName="graphiccard";
        $graphiccard->accessoriesCategory="Accessories";

        $graphiccard->device_pic=$device_pic ?? '';
        $graphiccard->invoice_file=$invoice_file ?? '';
        $graphiccard->accessories_type='5';
        //$graphiccard->system_type='1';
        $graphiccard=$graphiccard->save();
        if($graphiccard){
            $graphiccard_data=Asset::orderBy('id','desc')->first()->id;
            $assignIT=Employee::where('role','AdminIT')->where('status','1')->first()->id;
            $assign=new Assign;
            $assign->type="7";
            //$assign->graphiccard=$request->input('graphiccard');
            $assign->item_id=$graphiccard_data;
            ...
            $assign=$assign->save();
            if($assign){
                return back()->with('msg',"graphiccard Add Successfully");
            }else{
                return back()->with('err_msg',"Some Error");
            }
            
        }else{
            return back()->with('err_msg',"Some Error in graphiccard Add");
        }
    }


    //graphiccard assign
    public function graphiccardAssign(Request $request){
    ...
    public function productDetail($id){
        ...
        $data->join('employee','employee.id','=','assign_table.assign');
        $data->where('assign_table.type','7');
        $data->where('ram.accessories_type','5');
